...
from models.yolo import Model, TSCODE_Detect, Yolov7_Tiny_E_ELAN_Attention, Yolov7_Tiny_E_ELAN_DCN, IDetect, IDetect_AFPN_P345, Yolov7_Tiny_E_ELAN_DBB, DiverseBranchBlock, \
    IDetect_Decoupled_DyHead, IDetect_Decoupled, ImplicitA, ImplicitM,Yolov7_Tiny_E_ELAN_DCN_CBAM,Yolov7_Tiny_SPP_CBAM,FT_Conv
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, Yolov7_Tiny_E_ELAN_DCN):
            ignored_layers.append(m.conv3)
            ignored_layers.append(m.conv4)
        if isinstance(m, FT_Conv):
            ignored_layers.append(m)
            # ignored_layers.append(m.PWC1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/root/autodl-tmp/runs/train/yolov7-tiny-coupling-22-10/weights/best.pt', help='trained weights path')
    ...
    parser.add_argument('--name', default='group_norm-2.0', help='save to project/name')
    ...
    # prune
    parser.add_argument('--prune_method', type=str, default='group_norm', help='prune method')
    parser.add_argument('--speed_up', type=float, default=2, help='speed up')
    ...
    parser.add_argument("--sl_epochs", type=int, default=400)
    parser.add_argument("--sl_model", type=str, default="/root/autodl-tmp/runs/compress/slim-2.5_sl2/weights/best_sl_0.331.pt", help='sparsity learning trained model weights')
