...
if __name__ == '__main__':
    ...
    parser.add_argument('--base-weights', type=str, default='/root/autodl-tmp/yolov7-prune-main-1/runs/train/10/weights/best.pt', help='base weights path')
    parser.add_argument('--prune-weights', type=str, default='/root/autodl-tmp/yolov7-prune-main-1/runs/compress/slim-F-10-2.0-12/weights/best.pt', help='prune weights path')
    opt = parser.parse_args()
    
    print(f'loading base model from {opt.base_weights}')
    base_model = attempt_load(opt.base_weights, map_location=torch.device('cpu'))
    print(f'loading prune model from {opt.prune_weights}')
    prune_model = attempt_load(opt.prune_weights, map_location=torch.device('cpu'))
