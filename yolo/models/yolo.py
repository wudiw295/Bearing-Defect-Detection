...
class IDetect_Decoupled_DyHead(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export
    end2end = False
    include_nms = False
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        super(IDetect_Decoupled_DyHead, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
...
class IDetect_DyHead_Prune(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export
    end2end = False
    include_nms = False
    ...
    def __init__(self, nc=80, anchors=(), ch=(), hidc=128):  # detection layer
        super(IDetect_DyHead_Prune, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, TSCODE_Detect):
            s = 640  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s))] + [64])  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)[:-1]
            self.stride = m.stride
            self._initialize_biases_tscode()  # only run once
            # print('Strides: %s' % m.stride.tolist())
        # if isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled,IDetect_DyHead_Prune)):
        #     s = 640  # 2x min stride
        ...
        if isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled,IDetect_DyHead_Prune)):
            ...
            self.model.to(device)  # 将模型移到设备上
            # with torch.no_grad():  # 不需要梯度
                # 使用相同设备计算 m.stride..
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s).to(device))])
            check_anchor_order(m)
            # 将 anchors 移到相同设备上并执行操作
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled,IDetect_DyHead_Prune)) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled, IAuxDetect, IBin, TSCODE_Detect,IDetect_DyHead_Prune))
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        if isinstance(m, IDetect):
            for mi, s in zip(m.m, m.stride):  # from
                ...
                mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
        elif isinstance(m, (IDetect_Decoupled_DyHead, IDetect_Decoupled)):
            for mi, s in zip(m.m_conf, m.stride):  # from
                b = mi.bias.view(m.na, -1)  # conv.bias(255) to (3,85)
                b.data += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
                mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IAuxDetect, IDetect_Decoupled_DyHead, IDetect_Decoupled,IDetect_DyHead_Prune)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 SPP, SPPF_CPMS, SPPCSPC, GhostSPPCSPC, MixConv2d, Focus, Stem, GhostStem, CrossConv,
                 Bottleneck, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC,
                 RepBottleneck, RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC,  
                 Res, ResCSPA, ResCSPB, ResCSPC, DualConv,
                 RepRes, RepResCSPA, RepResCSPB, RepResCSPC, FSC,Yolov7_Tiny_E_ELAN_EMCA,
                 ResX, ResXCSPA, ResXCSPB, ResXCSPC, Yolov7_Tiny_E_ELAN_DCN_SaE,Yolov7_Tiny_E_ELAN_DCN_OSRA,
                 RepResX, RepResXCSPA, RepResXCSPB, RepResXCSPC,
                 Ghost, GhostCSPA, GhostCSPB, GhostCSPC,Yolov7_Tiny_SPP_BSMS,Yolov7_Tiny_E_ELAN_DCN1,
                 SwinTransformerBlock, STCSPA, STCSPB, STCSPC,Yolov7_Tiny_E_ELAN_DCN_BSAM,Yolov7_Tiny_SPP_BSAM,Yolov7_Tiny_E_ELAN_DCN_CBAM1,
                 SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC,PConv,Yolov7_Tiny_E_ELAN_DBB_LSKB,Yolov7_Tiny_E_ELAN_DCN_CPMS,
                 Yolov7_Tiny_E_ELAN, Yolov7_Tiny_SPP, Yolov7_E_ELAN, Yolov7_E_ELAN_NECK,SPPCSPC_CBAM,Yolov7_Tiny_SPP_MSAM,Yolov7_Tiny_SPP_CAMS,
                 Yolov7x_E_ELAN, Yolov7_Tiny_E_ELAN_Attention, Yolov7_Tiny_E_ELAN_DCN,Yolov7_Tiny_SPP_CBAM,Yolov7_Tiny_E_ELAN_DCN_SEAttention,Yolov7_Tiny_SPP_SEAttention,
                 Yolov7_Tiny_E_ELAN_DBB, DiverseBranchBlock,Yolov7_Tiny_E_ELAN_DBB_CBAM,Yolov7_Tiny_E_ELAN_DCN_MSAM,Yolov7_Tiny_E_ELAN_DCN_CBAM,Yolov7_Tiny_E_ELAN_PConv,VoVGSCSP, VoVGSCSPC,GSConvns,RFEM]:
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f] // 2
        elif m in [Detect, TSCODE_Detect, IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled, IAuxDetect, IBin, IKeypoint]:
            ...
            t = m
            if len(args) == 2:
                m = timm.create_model(m, pretrained=args[0], out_indices=args[1], features_only=True)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/root/autodl-tmp/cfg/training/yolov7-tiny-rexnet.yaml', help='model.yaml')
