...
from models.yolo import Model, TSCODE_Detect, Yolov7_Tiny_E_ELAN_Attention, Yolov7_Tiny_E_ELAN_DCN_CBAM, IDetect, IDetect_AFPN_P345, Yolov7_Tiny_E_ELAN_DBB, DiverseBranchBlock, \
    IDetect_Decoupled_DyHead, IDetect_Decoupled, ImplicitA, ImplicitM,Yolov7_Tiny_E_ELAN_DCN,Yolov7_Tiny_SPP_CBAM
...
from MyPrune import DiverseBranchBlockPruner
def get_pruner(opt, model, example_inputs):
    ...
    # ignored_layers.append(model.model[77])
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, Yolov7_Tiny_E_ELAN_DCN_CBAM):
            ignored_layers.append(m.conv3)
            ignored_layers.append(m.conv4)
            ignored_layers.append(m.conv6)
        if isinstance(m, Yolov7_Tiny_SPP_CBAM):
            ignored_layers.append(m.conv5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='runs/train/yolov7-tiny-coupling-22/weights/best.pt', help='trained weights path')
    ...
    parser.add_argument('--project', default='runs/compress', help='save to project/name')
    parser.add_argument('--entity', default=None, help='W&B entity')
    parser.add_argument('--name', default='yolov7-tiny-coupling-22-slim-500-3.0', help='save to project/name')
    ...
    # prune
    parser.add_argument('--prune_method', type=str, default='slim', help='prune method')
    parser.add_argument('--speed_up', type=float, default=3, help='speed up')
    ...
    # sparsity learning
    parser.add_argument("--reg", type=float, default=5e-3)
    ...
    parser.add_argument("--sl_epochs", type=int, default=500)
    parser.add_argument("--sl_model", type=str, default="/root/autodl-tmp/runs/compress/yolov7-tiny-coupling-22-slim-500-2.5_sl/weights/best_sl_0.388.pt", help='sparsity learning trained model weights')
    ...
    # parser.add_argument("--reg_decay", type=float, default=0.1)
    ...
    # pruning
    model = model_prune(opt, model, prune, example_inputs, testloader, imgsz_test)
