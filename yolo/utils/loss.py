...
from utils.general import bbox_mpdiou
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy,bbox_inner_mpdiou
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_inner_mpdiou(pbox, tbox[i], xywh=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2,ratio=1.27)
                ...
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * iou.detach().clamp(0).squeeze().type(tobj.dtype)
                # Classification
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_inner_mpdiou(pbox, selected_tbox, xywh=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2,ratio=1.27)
                # iou = bbox_mpdiou(pbox, selected_tbox, xywh=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2)
                # iou = bbox_mpdiou(pbox.T, selected_tbox, x1y1x2y2=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2,
                #                   grid=torch.stack([gj, gi]))  # iou(prediction, target)
